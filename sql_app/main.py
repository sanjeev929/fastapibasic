...
app = FastAPI(
    ...
    summary="Python's favorite app. Nuff said.",
    version="0.0.1",
...
@app.post("/dusers/", response_model=schemas.Userd)
...
@app.get("/dusers/", response_model=list[schemas.Userd])
def read_users(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    users = crud.get_users(db, skip=skip, limit=limit)
    return users


@app.get("/dusers/{user_id}", response_model=schemas.Userd)
...
@app.post("/dusers/{user_id}/items/", response_model=schemas.Itemd)
...
@app.get("/ditems/", response_model=list[schemas.Itemd])
